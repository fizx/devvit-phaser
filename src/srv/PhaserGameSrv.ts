 ...
 */
export class PhaserGameSrv extends BasicGameSrv {
  ...
  override async onPlayerJoined(): Promise<any> {
    ...
    const info = new DataManagerSrv(this, { id: "players" });
  ...
  async subscribePlayerToDataManager(dataManager: DataManagerSrv) {
    await this.processDataManagerSubscription({
      add: [dataManager.id],
    });
  }
  async unsubscribePlayerFromDataManager(dataManager: DataManagerSrv) {
    await this.processDataManagerSubscription({
  ...
  async processDataManagerMutation(mutation: DataManagerMutation) {
    ...
    const server = new DataManagerSrv(this, mutation.dataManagerId);
